...
echo " 1. Version changes to the codebase (optionally adding the .docksal directory)"
echo " 2. Rsync all files to the destination site: https://github.austin.utexas.edu/eis1-wcs/utexas_migrate/wiki/SFTP-all-files-from-a-local-migration-to-Pantheon"
echo " 3. Export the database (fin db dump $DESTINATION_SITE.sql)"
echo " 4. Import the database to Pantheon (terminus import:database <site>.<env> <url>)"
echo " 5. Add SAML assets (https://github.austin.utexas.edu/eis1-wcs/pantheon_saml_integration/issues/11)"
echo " 6. Tag the site in the Pantheon dasboard with 'SAML'"
echo " 7. Sign in and change the site email and user 1 email to wcs-drupal-site-admins@utlists.utexas.edu"
